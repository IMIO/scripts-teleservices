pipeline {
        ...
        stage('Push deb to buster apt repo') {
            ...
            steps {
                withCredentials([usernameColonPassword(credentialsId: 'nexus-teleservices', variable: 'CREDENTIALS'),string(credentialsId: 'nexus-url-buster', variable:'NEXUS_URL_BUSTER')]) {
                    sh ('curl -vk --fail -u $CREDENTIALS -X POST -H Content-Type:multipart/form-data --data-binary @scripts-teleservices_`echo ${VERSION}`_amd64.deb $NEXUS_URL_BUSTER')
                }
            }
        }
    ...
    post {
        always {
            sh "rm -f scripts-teleservices_*.deb"
        }
