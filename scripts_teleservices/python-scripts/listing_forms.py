...
for formdef in FormDef.select(order_by="category_id"):
    ...
    html = "{}<h3 style='margin-left:2em;'>{} ({})</h2>".format(
        html, formdef.name, "PUBLIE" if formdef.disabled is True else "DESACTIVE"
    )
    ...
    for field in formdef.fields:
        if (
            hasattr(field, "data_source")
            and field.data_source is not None
            and len(field.data_source) > 0
            and field.data_source.get("type") not in datasources_existantes
        ):
            datasources_existantes.append(field.data_source.get("type"))
            html = "{}<li>{}{}</li>".format(
                html,
                field.data_source.get("type"),
