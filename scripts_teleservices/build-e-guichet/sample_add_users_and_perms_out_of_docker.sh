...
for commune in 'silly' 'olln' 'lierneux' 'huy' 'ecaussinnes' 'honnelles' 'courcelles' 'boussu'
    do
            ...
            docker exec -ti $commune"teleservices_"$commune"teleservices_1" sudo -u  wcs wcsctl -f /etc/wcs/wcs-au-quotidien.cfg runscript --vhost=$commune"-formulaires.guichet-citoyen.be" /opt/publik/scripts/scripts_teleservices/build-e-guichet/import-permissions.py full
    done
